...
class CAttributeNotFound(CBaseException):
    def __init__(self, ctx, clazz_map: ClazzMap, attribute: str):
        super().__init__(ctx)
...
class CMethodNotFound(CBaseException):
    def __init__(self, ctx, clazz_map: ClazzMap, attribute: str):
        super().__init__(ctx)
        self.clazz_map = clazz_map
        self.attribute = attribute
        ...
        self.new_line()
        self.error += f'Error <CMethodNotFound: Line {self.line}>: '

        self.error += f'class `{self.clazz_map.name}` does not have attribute `{self.attribute}`'
        match = self.closest_match(
            self.attribute,
            self.clazz_map.methods.keys()
        )
        if match:
            self.error += f'\nDid you mean `{match}`?'
